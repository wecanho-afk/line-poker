...
`;
app_js = app_js.replace(/    triggerBotIfNeeded\(\) \{[\s\S]*?1800\);\n        \}\n    \}/, start_timer_func);

app_js = app_js.split("this.triggerBotIfNeeded();").join("this.startTurnTimer();");
...
// 2. Update index.html
let idx_html = fs.readFileSync('index.html', 'utf-8');
...
`;
idx_html = idx_html.replace(/<button id="voice-toggle" .*?<\/button>/, voice_btn_html);
...
idx_html = idx_html.replace(/function toggleVoice\(\) \{[\s\S]*?\}\n/, "");
idx_html = idx_html.replace(/\/\/ Voice synthesis for actions[\s\S]*?\}\n            \}/, "");

fs.writeFileSync('index.html', idx_html, 'utf-8');
